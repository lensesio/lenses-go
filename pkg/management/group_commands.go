...
groups get --name=MyGroup --namespaces
`,
		TraverseChildren: true,
		SilenceErrors:    true,
		RunE: func(cmd *cobra.Command, args []string) error {
	...
	cmd.Flags().BoolVar(&namespaceOnly, "namespaces", false, `Print namespaces only`)
...
func NewCreateGroupCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `
groups create --name MyGroup --description "My test group" --scopedPermissions ViewKafkaConsumers --scopedPermissions ManageKafkaConsumers --scopedPermissions ViewConnectors --namespaces '[{"wildcards":["*"],"permissions":["CreateTopic","DropTopic","ConfigureTopic","QueryTopic","ShowTopic","ViewTopicMetadata","InsertData","DeleteData","UpdateTablestore"],"system":"Kafka","instance":"Dev"}]'
`,
		TraverseChildren: true,
		SilenceErrors:    true,
		RunE: func(cmd *cobra.Command, args []string) error {
...
func NewUpdateGroupCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `
groups update --name MyGroup --description "My test group" --scopedPermissions ViewKafkaConsumers --scopedPermissions ManageKafkaConsumers --scopedPermissions ViewConnectors --namespaces '[{"wildcards":["*"],"permissions":["CreateTopic","DropTopic","ConfigureTopic","QueryTopic","ShowTopic","ViewTopicMetadata","InsertData","DeleteData","UpdateTablestore"],"system":"Kafka","instance":"Dev"}]'
`,
		TraverseChildren: true,
		SilenceErrors:    true,
		RunE: func(cmd *cobra.Command, args []string) error {
...
func validateCreateUpdate(cmd *cobra.Command, namespacesRaw *string, group *api.Group) error {
	...
	if len(group.ScopedPermissions) == 0 {
		return fmt.Errorf("required flag \"scopedPermissions\" not set")
	}
	if *namespacesRaw != "" {
		if err := bite.TryReadFile(*namespacesRaw, &group.Namespaces); err != nil {
			...
			if err = json.Unmarshal([]byte(*namespacesRaw), &group.Namespaces); err != nil {
				return fmt.Errorf("Unable to unmarshal the namespaces: [%v]", err)
			}
		}
	}
	return nil
...
func addCreateUpdateFlags(cmd *cobra.Command, namespacesRaw *string, group *api.Group) {
	...
	cmd.Flags().StringArrayVar(&group.ScopedPermissions, "scopedPermissions", []string{}, "Group scoped permissions")
	cmd.Flags().StringArrayVar(&group.AdminPermissions, "adminPermissions", []string{}, "Group admin permissions")
	cmd.Flags().StringVar(namespacesRaw, "namespaces", "", `Group namespaces: "[{"wildcards":["*"],"permissions":["CreateTopic","DropTopic","ConfigureTopic","QueryTopic","ShowTopic","ViewTopicMetadata","InsertData","DeleteData","UpdateTablestore"],"system":"Kafka","instance":"Dev"}]"`)
