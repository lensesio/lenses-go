...
func NewCreateServiceAccountCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `
serviceaccounts create --name john --owner admin --groups MyGroup1 --groups MyGroup2 [--token <token>]
`,
		TraverseChildren: true,
		SilenceErrors:    true,
		RunE: func(cmd *cobra.Command, args []string) error {
...
func addCreateUpdateSvcAccFlags(cmd *cobra.Command, svcacc *api.ServiceAccount) {
	...
	cmd.Flags().StringVar(&svcacc.Token, "token", "", "Your own manual service account token. Otherwise will be autogenerated")
