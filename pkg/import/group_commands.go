...
func loadGroups(client *api.Client, cmd *cobra.Command, loadpath string) error {
	...
	for _, file := range files {
		...
		if err := bite.LoadFile(cmd, fmt.Sprintf("%s/%s", loadpath, file.Name()), &groups); err != nil {
		...
		for _, g := range groups {
			...
			if err := client.CreateGroup(&g); err != nil {
				golog.Errorf("Error creating user group [%s] from [%s] [%s]", g.Name, loadpath, err.Error())
				return err
			}
		}
	}
	golog.Infof("Created/updated groups from [%s]", loadpath)
