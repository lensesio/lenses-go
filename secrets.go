...
const AzureKeyVaultDNS = "https://vault.azure.net"
...
func getVaultClient(server, token string) (*vaultapi.Client, error) {
	...
	if config.Address == "" {
		if server == "" {
			return nil, errors.New(`required flag(s) "vault-addr" not set and no VAULT_ADDR environment variable found`)
		}
	...
	if envToken := os.Getenv(vaultapi.EnvVaultToken); envToken == "" {
		if token == "" {
			return nil, errors.New(`required flag(s) "vault-token" not set and no VAULT_TOKEN environment variable found`)
		}
...
func newKeyVaultClient(vaultURL string, config AzureConfiguration) (*KeyVault, error) {
	...
	credentials.Resource = AzureKeyVaultDNS
